...
class MCTS_Searcher:
    ...
    def _select(self, node: MCTS_Node, rollout_id: int) -> List[MCTS_Node]:
        ...
        while True:
            ...
            unexplored = [n for n in self.parent2children[node] if n not in self.explored_nodes and n.node_type is not Node_Type.DIRECT_ANSWER]
            if unexplored:
    ...
    def _simulate(self, node: MCTS_Node, rollout_id: int) -> List[MCTS_Node]:
        ...
        while True:
            ...
            for child in self.parent2children[cur_node]:
                if child.sentence_num is not None and child.sentence_num > 4:
                    continue
                selected_node_list.append(child)
            cur_node = random.choice(selected_node_list)  # randomly select a child
    ...
    def _uct_select(self, node: MCTS_Node, rollout_id: int):
        ...
        assert all(n in self.explored_nodes for n in self.parent2children[node] if n.node_type is not Node_Type.DIRECT_ANSWER)
        
        node_to_select = [n for n in self.parent2children[node] if n.node_type is not Node_Type.DIRECT_ANSWER]
        return max(
            node_to_select, key=lambda n: self._compute_uct(parent_node=node, node=n, rollout_id=rollout_id)
        )
