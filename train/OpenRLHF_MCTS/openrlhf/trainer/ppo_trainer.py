...
class PPOTrainer(ABC):
    ...
    def fit(
        ...
        for episode in range(start_episode, args.num_episodes):
            ...
            for rand_prompts, rand_sol, rand_id in self.prompts_dataloader:
                for i, experience in enumerate(
                    self.experience_maker.make_experience_list(rand_prompts, rand_sol, rand_id, **self.generate_kwargs)
                ):
                    if i == 0:
    ...
    def save_logs_and_checkpoints(self, args, global_step, step_bar, logs_dict={}, client_states={}):
        if global_step % args.logging_steps == 0:
            ...
            with open(args.logging_path, 'a+', encoding='utf-8') as f:
                import json
                json.dump(logs, f)
                f.write('\n')
            # wandb
    ...
    def _save_checkpoint(self, args, tag, client_states):
        ...
        self.strategy.save_model(
            ...
            self.tokenizer,
            os.path.join(args.ckpt_path, f"_actor/{tag}"),
        )
        if self.critic is not None:
            self.strategy.save_ckpt(
